...
class PgPersistence {
  ...
  async authenticateUsername(username, password) {
    const FIND_HASHED_PASS = `SELECT *
                              FROM users
                              WHERE username = $1`;

    let result = await dbQuery(FIND_HASHED_PASS, username);
  ...
  async loadAlbum(albumId) {
    ...
    let resultAlbum = dbQuery(FIND_ALBUM, albumId, this.username);
    let resultImages = dbQuery(FIND_IMAGES, albumId, this.username);
    let resultBoth = await Promise.all([resultAlbum, resultImages]);

    let album = resultBoth[0].rows[0];
    if (!album) return false;

    album.images = resultBoth[1].rows;
    return album;
  }
  ...
  async sortedAlbums(pageNumber = 1) {
    ...
    let resultAlbums = dbQuery(ALL_ALBUMS, offset, this.username);
    let resultImages = dbQuery(ALL_IMAGES, this.username);
    let resultBoth = await Promise.all([resultAlbums, resultImages]);

    let allAlbums = resultBoth[0].rows;
    let allImages = resultBoth[1].rows;
